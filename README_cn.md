...
面向线缆巡检场景的 ROS2/TROS 开源项目，基于“地瓜机器人 RDK X5 | 极致易用的机器人开发平台”构建。机器人采用差分轮底盘沿线巡检，通过 RGB 摄像头识别线缆中心线和缺陷候选区域，可选接入 LiDAR、IMU 与轮速里程计完成避障、建图和定位。项目代码覆盖“感知 - 定位 - 规划 - 控制 - 记录”的闭环，便于在 RDK X5 和桌面 ROS2 环境复现核心流程。
...
实际样机可按底盘尺寸、镜头视场、巡检线宽和地面摩擦条件重新标定。
...
代码采用 Apache License 2.0 开源。模型权重、第三方数据集、硬件厂商资料和相关文档如另有授权，应遵循其原始许可证。

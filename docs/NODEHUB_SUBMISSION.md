...
本项目面向线缆巡检场景，构建了一套基于地瓜机器人平台、可复现的线缆巡检机器人方案。系统以 RDK X5 为主计算平台，运行 ROS2/TROS 感知、规划和日志节点；WHEELTEC C30D ROS 四驱主控板承担底盘电机闭环和差分轮执行；RGB 摄像头用于实时检测线缆偏移和缺口、弯折、破损等异常；可选 LiDAR、IMU 与轮速里程计用于避障、建图和定位。
